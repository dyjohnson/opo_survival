...
Our project used the quarter 2 2025 SAF:
```{r read_in_SAS}

# read in the SRTR SAF files
cand_thor <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/cand_thor.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

tx_hr <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/tx_hr.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

stathist_thor <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/stathist_thor.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

statjust_hr1a <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/statjust_hr1a.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

statjust_hr1b <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/statjust_hr1b.sas7bdat', NULL) %>% 
  zap_formats() %>% zap_labels()

donor_deceased <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/donor_deceased.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

institution <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/institution.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()

hist_opo_txc <- read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/hist_opo_txc.sas7bdat', NULL) %>%  
  zap_formats() %>% zap_labels()
```
...
#add OPO
init_list <- init_list %>%
  left_join(hist_opo_txc, 
            by = c("CAN_LISTING_CTR_CD" = "TXC_CTR_CD",
                   "CAN_LISTING_CTR_TY" = "TXC_CTR_TY")) %>%
  filter(list_date >= START_DT & list_date <= END_DT) %>%
  mutate(OPO = SERVED_OPO_CD)
...
init_list <- init_list %>% 
  mutate(transplant = ifelse(CAN_REM_CD == 4, 1, 0)) %>%
	group_by(CAN_LISTING_CTR_CD, CAN_LISTING_CTR_TY) %>% 
...
init_list <- init_list %>% 
  select(PX_ID, OPO, CAN_LISTING_CTR_CD, CAN_LISTING_CTR_TY, status, 
...
tx <- tx %>% select(PX_ID, REC_OPO_ID, REC_CTR_CD, REC_CTR_TY, DONOR_ID, status, tx_date, CAN_RACE,
...
just_1a <- just_1a %>% select(PX_ID, CAN_LISTING_CTR_CD, CAN_LISTING_CTR_TY, 
...
just_1b <- just_1b %>% 
  select(PX_ID, CAN_LISTING_CTR_CD, CAN_LISTING_CTR_TY, status, date_start, 
...
ts <- ts %>% mutate(
  center_cd = case_when(
  	is.na(CAN_LISTING_CTR_CD.init) == FALSE ~ CAN_LISTING_CTR_CD.init,
  	is.na(CAN_LISTING_CTR_CD.j1a) == FALSE ~ CAN_LISTING_CTR_CD.j1a,
  	is.na(REC_CTR_CD) == FALSE ~ REC_CTR_CD
	), 
  center_cd = na.locf(center_cd),
  center_ty = case_when(
  	is.na(CAN_LISTING_CTR_TY.init) == FALSE ~ CAN_LISTING_CTR_TY.init,
  	is.na(CAN_LISTING_CTR_TY.j1a) == FALSE ~ CAN_LISTING_CTR_TY.j1a,
  	is.na(REC_CTR_TY) == FALSE ~ REC_CTR_TY
	), 
  center_ty = na.locf(center_ty))

ts <- ts %>%
  left_join(institution, 
            by = c("REC_CTR_CD" = "CTR_CD",
                   "REC_CTR_TY" = "CTR_TY")) %>%
  mutate(center = CTR_ID)

hist_opo_txc_clean <- hist_opo_txc %>%
  arrange(TXC_CTR_CD, TXC_CTR_TY, SERVED_OPO_CD, SERVED_OPO_TY, START_DT, END_DT) %>%
  group_by(TXC_CTR_CD, TXC_CTR_TY, SERVED_OPO_CD, SERVED_OPO_TY) %>%
  mutate(
    # Start a new group whenever the current interval does NOT overlap/meet the previous
    new_group = cumsum(
      START_DT > lag(END_DT, default = first(END_DT))
    )
  ) %>%
  group_by(TXC_CTR_CD, TXC_CTR_TY, SERVED_OPO_CD, SERVED_OPO_TY, new_group) %>%
  summarise(
...
ts <- ts %>%
  left_join(hist_opo_txc_clean,
            by = c("REC_CTR_CD" = "TXC_CTR_CD",
                   "REC_CTR_TY" = "TXC_CTR_TY")) %>%
  filter(tx != 1 | (list_date >= START_DT & list_date <= END_DT)) %>%
  mutate(REC_OPO = SERVED_OPO_CD)


ts <- ts %>% mutate(
  OPO = case_when(
	is.na(REC_OPO) == FALSE ~ REC_OPO,
	TRUE ~ OPO
	),  OPO = na.locf(OPO))
...
overlaps <- hist_opo_txc %>%
  arrange(TXC_CTR_CD, TXC_CTR_TY, SERVED_OPO_CD, SERVED_OPO_TY, START_DT, END_DT) %>%
  group_by(TXC_CTR_CD, TXC_CTR_TY) %>%
  mutate(
    next_start = lead(START_DT),
    next_end   = lead(END_DT),
    overlap = !is.na(next_start) & START_DT <= next_end & END_DT >= next_start
  ) %>%
  filter(overlap == TRUE) %>%
  select(TXC_CTR_CD, TXC_CTR_TY, SERVED_OPO_CD, SERVED_OPO_TY,
         START_DT, END_DT,
         overlapped_with_start = next_start,
         overlapped_with_end   = next_end)
