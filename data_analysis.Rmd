...
```{r data_in}
to_model <- read.csv("/Users/danieljohnson/Library/CloudStorage/OneDrive-TheUniversityofChicago/Projects/OPO/opo_survival/clean_time_series_revise2006_2015.csv")
...
six_status <- "Surv(t_1, t_2, dead) ~ tx*(status_2 + status_3 + status_4 + status_6) + tx_risky_don + era_tx"
```
...
d_ij <- d_ij %>% mutate(
	...
	group_by(tx) %>% 
	mutate(
		surv_post_tx = if_else( tx ==1, cumprod(1-OPO_hz_tx), NULL), 
...
o_list <- row.names(ranef(three_status)[[1]])
