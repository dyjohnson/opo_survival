...
```{r data_in}
to_model <- read.csv("/Users/danieljohnson/Library/CloudStorage/OneDrive-TheUniversityofChicago/Projects/OPO/opo_survival/clean_time_series_revise2018_2022.csv")

# Convert OPO to factor if it isn't already, to ensure correct handling in mixed models
to_model$OPO <- as.factor(to_model$OPO)

#for table S4
tx_hr <- haven::read_sas("/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/tx_hr.sas7bdat", NULL) %>%  
...
# three_status <- "Surv(t_1, t_2, dead) ~ tx*(stat_2 + stat_1b) + tx_risky_don + era_tx"

#six-status, donor quality and transplant year
six_status <- "Surv(t_1, t_2, dead) ~ tx*(status_2 + status_3 + status_4 + status_6) + tx_risky_don"
```



```{r run_fe_cox_models}
formulas <- c(six_status)
...
for (i in seq_along(formulas)) {
  f_coxme <- as.formula(paste(formulas[[i]], " + (1+ tx|OPO)"))
  start_time <- Sys.time()
  six_status <- coxme(f_coxme, data = to_model)
  me_models[[i]] <- six_status 
  end_time <- Sys.time()
...
six_status <- me_models[[1]]

basehz_six_stat <- hz_list[[1]]
...
```{r hr_tx_ij}
hr_tx_ij <- function(model = six_status, df= to_model, i, j){
  #this function returns the log hazard ratio of transplant 
  #for patient j transplanted at OPO i for a given coxme model  
  fixed_effects <- fixef(model)
  x_vars <- names(fixed_effects)
  ref <- data.frame(ranef(model)[[1]])
  ref$OPO <- row.names(ref)
  tx_ref <- filter(ref, OPO == i)$tx
...
fixed_effects <- fixef(model)
x_vars <- names(fixed_effects)
base_vars <- x_vars[!grepl(":", x_vars)]
ref <- data.frame(ranef(model)[[1]])
ref$OPO <- row.names(ref)

d_ij <- df %>% filter(PX_ID == j) %>% 
  select(PX_ID, t_1, t_2, dead, one_of(base_vars)) %>% 
  mutate(time = t_1, 
...
OPO_i_b0 <- ref[ref$OPO == i,]$Intercept
OPO_i_b1 <- ref[ref$OPO == i,]$tx

d_ij <- d_ij %>% mutate(
	fe_wait = lp_no_tx,
	fe_tx = fe_lp,
	OPO_hz_wait = hz_t*exp(fe_wait + OPO_i_b0),
	OPO_hz_tx = hz_t*exp(fe_tx + OPO_i_b0 + OPO_i_b1)
	) %>%
	mutate(wait_surv = cumprod(1-OPO_hz_wait)) %>% 
	group_by(tx) %>% 
	mutate(
		surv_post_tx = if_else( tx ==1, cumprod(1-OPO_hz_tx), NA_real_), 
...
# 	group_by(tx) %>%
# 	mutate(
# 		surv_post_tx = if_else( tx ==1, cumprod(1-OPO_hz_tx), rep(NA_real_, n())),
# 		surv_post_tx = max(wait_surv)*surv_post_tx) %>% ungroup()
# 
# 
# d_ij <- d_ij %>%
...
#   fixed_effects <- fixef(model)
#   x_vars <- names(fixed_effects)
#   # random effects frame — ensure column names are what we expect
#   ref <- as.data.frame(ranef(model)[[1]])
#   ref$OPO <- row.names(ranef(model)[[1]])
# 
#   d_ij <- df %>%
#     filter(PX_ID == j) %>%
#     select(PX_ID, t_1, t_2, dead, one_of(x_vars)) %>%
...
#            lp_no_tx = 0)
# 
#   # build linear predictors from fixed effects
#   for (x in x_vars) {
#     fe_x <- unname(fixed_effects[x])
# 
#     if (!grepl("tx:", x)) {
...
# 
#   ### Hypothetical when patient has always been transplanted
#   non_tx_terms <- x_vars[!grepl("tx", x_vars)]
#   for (x in non_tx_terms) {
#     fe_x <- unname(fixed_effects[x])
#     d_ij$c_x <- d_ij[[x]]
#     d_ij <- d_ij %>% mutate(lp_no_tx = lp_no_tx + c_x * fe_x) %>% select(-c_x)
#   }
# 
#   # join baseline hazard by time
#   d_ij <- left_join(base_hz, d_ij, by = c("time")) %>%
#     select(-t_1, -t_2)
# 
#   # carry forward NAs (as before)
#   d_ij <- d_ij %>% mutate_all(~ zoo::na.locf(., na.rm = FALSE))
# 
#   # keep only person rows where tx == 1 and rebase time to transplant
#   d_ij <- d_ij %>% filter(tx == 1)
#   if (nrow(d_ij) == 0) {
#     warning(paste0("Candidate ", j, " not transplanted"))
#     return(c(NA_real_, NA_real_))
#   }
#   d_ij <- d_ij %>% mutate(time = time - min(time) + 1)
# 
#   # Extract OPO random effect(s) safely and coerce to single scalars
#   row_ref <- ref[ref$OPO == as.character(i), , drop = FALSE]
# 
#   if (nrow(row_ref) == 0) {
#     # no random effect for this OPO — treat as zero (or you can return NA)
#     OPO_i_b0 <- 0
#     OPO_i_b1 <- 0
#     warning(paste0("No random effect found for OPO = ", as.character(i),
#                    ". Using 0 for intercept and tx RE."))
#   } else {
#     # if multiple rows found for the OPO, pick the first and warn
#     if (nrow(row_ref) > 1) {
#       warning(paste0("Multiple random-effect rows found for OPO = ", as.character(i),
#                      ". Using the first row."))
#     }
#     # rename defensive: allow both "Intercept"/"tx" or "(Intercept)"/"tx"
#     if ("Intercept" %in% colnames(row_ref)) {
#       OPO_i_b0 <- as.numeric(row_ref$Intercept[1])
#     } else if ("(Intercept)" %in% colnames(row_ref)) {
#       OPO_i_b0 <- as.numeric(row_ref[["(Intercept)"]][1])
#     } else {
#       stop("Unexpected ranef column name for intercept; expected 'Intercept' or '(Intercept)'.")
#     }
#     # tx random effect
#     if ("tx" %in% colnames(row_ref)) {
#       OPO_i_b1 <- as.numeric(row_ref$tx[1])
#     } else {
#       OPO_i_b1 <- 0
#       warning("No 'tx' random-effect column found; using 0 for OPO tx effect.")
#     }
#   }
# 
#   # now compute hazards and survivals
#   d_ij <- d_ij %>% mutate(
#     fe_wait = lp_no_tx,
#     fe_tx = fe_lp,
#     # hz_t exists from base_hz join
#     OPO_hz_wait = hz_t * exp(fe_wait + OPO_i_b0),
#     OPO_hz_tx   = hz_t * exp(fe_tx + OPO_i_b0 + OPO_i_b1)
#   ) %>%
#     mutate(wait_surv = cumprod(1 - OPO_hz_wait)) %>%
...
#     mutate(
#       surv_post_tx = if_else(tx == 1, cumprod(1 - OPO_hz_tx), rep(NA_real_, n())),
#       surv_post_tx = max(wait_surv, na.rm = TRUE) * surv_post_tx
#     ) %>%
#     ungroup()
# 
#   # truncate to requested end time
#   d_ij <- d_ij %>% filter(time <= max_end_time) %>% mutate(surv_benefit = surv_post_tx - wait_surv)
# 
#   # return 2-element numeric: wait_5, post_5 (may be NA)
#   wait_5 <- d_ij %>% filter(time == end_times) %>% pull(wait_surv)
#   post_5 <- d_ij %>% filter(time == end_times) %>% pull(surv_post_tx)
#   if (length(wait_5) == 0) wait_5 <- NA_real_
#   if (length(post_5) == 0) post_5 <- NA_real_
# 
#   c(wait_5, post_5)
# }
```
...
```{r wait_tx_5_year_v3}
# # V3: data.table version for speed
# library(data.table)
# library(zoo)
# 
# # convert to data.table once
# dt_model <- as.data.table(to_model)
# 
# # use the baseline hazard you already built for the model (hz_list[[1]])
# # make sure basehz is a data.frame / data.table with columns time, hz_t
# basehz_dt <- as.data.table(hz_list[[1]])   # or base_hz used previously; ensure it's correct
# setkey(basehz_dt, time)
# 
# # fixed effects and ranef frame precomputed once
# fixed_effects_vec <- fixef(three_status)    # or me_models[[1]] if you used that model
# x_vars <- names(fixed_effects_vec)
# 
# ranef_df <- as.data.frame(ranef(three_status)[[1]])
# ranef_df$OPO <- row.names(ranef(three_status)[[1]])
# # ensure ranef_df columns named consistently: either "(Intercept)" or "Intercept"
# # standardize names:
# if ("(Intercept)" %in% colnames(ranef_df)) {
#   ranef_df <- rename(ranef_df, Intercept = "(Intercept)")
# }
# 
# # quick lookup table for recipients grouped by OPO (for tests)
# recips_dt <- unique(dt_model[tx == 1, .(PX_ID, OPO)])
# setkey(recips_dt, OPO)
# 
# ben_wait_tx_fast <- function(px_id,
#                              opo,
#                              dt_model,
#                              basehz_dt,
#                              fixed_effects_vec,
...
#                              end_times = 1825) {
# 
#   # fast subset (very cheap)
#   d_ij <- dt_model[PX_ID == px_id]
# 
#   if (nrow(d_ij) == 0) {
#     warning("No rows for PX_ID: ", px_id)
#     return(c(NA_real_, NA_real_))
#   }
# 
#   # keep only needed columns: time columns + covariates + tx
#   keep_cols <- unique(c("PX_ID", "t_1", "t_2", "dead", "tx", x_vars))
#   keep_cols <- intersect(keep_cols, colnames(d_ij))
#   d_ij <- d_ij[, ..keep_cols]
# 
#   # time variable
#   d_ij[, time := t_1]
#   d_ij[, time := fifelse(time == 0, 1L, time)]
# 
#   # If recipient never transplanted -> return NA
#   if (!any(d_ij$tx == 1, na.rm = TRUE)) {
#     # candidate not transplanted
#     return(c(NA_real_, NA_real_))
#   }
# 
#   # join baseline hazard quickly (many rows but basehz small)
#   d_ij <- merge(basehz_dt, d_ij, by = "time", all.x = FALSE, all.y = TRUE, sort = FALSE)
# 
#   # carry forward NAs for columns that matter (hz_t and other covariates)
#   covs_to_fill <- c("hz_t", intersect(x_vars, colnames(d_ij)), "tx")
#   for (col in covs_to_fill) {
#     if (col %in% colnames(d_ij)) d_ij[[col]] <- zoo::na.locf(d_ij[[col]], na.rm = FALSE)
#   }
# 
#   # keep only post-transplant rows (tx == 1) and rebase time
#   d_ij <- d_ij[tx == 1]
#   if (nrow(d_ij) == 0) return(c(NA_real_, NA_real_))
#   d_ij[, time := time - min(time) + 1L]
# 
#   # Build design matrix (vectorized)
#   # For each fixed effect name x in x_vars:
#   # - if it's an interaction like "tx:age", design column = age * tx
#   # - else design column = value of the variable
#   design_cols <- character(0)
#   design_mat <- matrix(0, nrow = nrow(d_ij), ncol = length(x_vars))
#   col_idx <- 1L
#   for (x in x_vars) {
#     if (grepl(":", x, fixed = TRUE)) {
#       parts <- strsplit(x, ":", fixed = TRUE)[[1]]
#       # assuming pattern tx:VAR or VAR:tx
#       if ("tx" %in% parts) {
#         other <- parts[parts != "tx"]
#         if (length(other) != 1) stop("Unexpected interaction term: ", x)
#         varname <- other
#         if (!(varname %in% colnames(d_ij))) {
#           # missing covariate, filled with zeros
#           design_mat[, col_idx] <- 0
#         } else {
#           design_mat[, col_idx] <- d_ij[[varname]] * d_ij[["tx"]]
#         }
#         design_cols <- c(design_cols, x)
#       } else {
#         # general interaction — try multiply columns if present
#         if (all(parts %in% colnames(d_ij))) {
#           tmp <- d_ij[[parts[1]]]
#           for (k in 2:length(parts)) tmp <- tmp * d_ij[[parts[k]]]
#           design_mat[, col_idx] <- tmp
#         } else {
#           design_mat[, col_idx] <- 0
#         }
#         design_cols <- c(design_cols, x)
...
#     } else {
#       # main effect
#       if (x %in% colnames(d_ij)) {
#         design_mat[, col_idx] <- d_ij[[x]]
#       } else {
#         design_mat[, col_idx] <- 0
#       }
#       design_cols <- c(design_cols, x)
...
#   }
# 
#   # vectorized linear predictor: fe_lp and lp_no_tx (hypothetical always transplanted)
#   # fe_lp = design_mat %*% fixed_effects_vec
#   fe_lp_vec <- as.numeric(design_mat %*% as.numeric(fixed_effects_vec))
#   # For lp_no_tx: zero out columns containing "tx" in their name
#   tx_cols <- grepl("tx", design_cols, fixed = TRUE)
#   design_mat_no_tx <- design_mat
#   if (any(tx_cols)) design_mat_no_tx[, tx_cols] <- 0
#   lp_no_tx_vec <- as.numeric(design_mat_no_tx %*% as.numeric(fixed_effects_vec))
# 
#   # attach to d_ij
#   d_ij[, fe_lp := fe_lp_vec]
#   d_ij[, lp_no_tx := lp_no_tx_vec]
# 
#   # get OPO random effect row (scalar intercept and tx)
#   row_ref <- ranef_df[ranef_df$OPO == as.character(opo), , drop = FALSE]
#   if (nrow(row_ref) == 0) {
#     OPO_i_b0 <- 0
#     OPO_i_b1 <- 0
#   } else {
#     if (nrow(row_ref) > 1) warning("Multiple ranef rows for OPO ", opo, " — using first")
#     OPO_i_b0 <- if ("Intercept" %in% colnames(row_ref)) as.numeric(row_ref$Intercept[1]) else as.numeric(row_ref[[1]][1])
#     OPO_i_b1 <- if ("tx" %in% colnames(row_ref)) as.numeric(row_ref$tx[1]) else 0
#   }
# 
#   # compute hazards and survival (vectorized)
#   d_ij[, OPO_hz_wait := hz_t * exp(lp_no_tx + OPO_i_b0)]
#   d_ij[, OPO_hz_tx   := hz_t * exp(fe_lp + OPO_i_b0 + OPO_i_b1)]
# 
#   # cumulative survival
#   d_ij[, wait_surv := cumprod(1 - OPO_hz_wait)]
#   # surv_post_tx (for tx rows)
#   d_ij[, surv_post_tx := cumprod(1 - OPO_hz_tx)]
#   # scale post_tx to start at same place as wait_surv at transplant time
#   max_wait <- max(d_ij$wait_surv, na.rm = TRUE)
#   d_ij[, surv_post_tx := max_wait * surv_post_tx]
# 
#   # truncate and compute benefit
#   d_ij <- d_ij[time <= end_times]
#   # pick the value at end_times (may be missing)
#   wait_5 <- d_ij[time == end_times, wait_surv]
#   post_5 <- d_ij[time == end_times, surv_post_tx]
#   if (length(wait_5) == 0) wait_5 <- NA_real_
#   if (length(post_5) == 0) post_5 <- NA_real_
#   c(wait_5, post_5)
# }
# 
# # pick test OPO
# test_opo <- unique(recips_dt$OPO)[1]
# test_recips <- recips_dt[OPO == test_opo, PX_ID]
# 
# # time the test
# system.time({
#   test_res_list <- sapply(test_recips, function(px) {
#     ben_wait_tx_fast(px_id = px,
#                      opo = test_opo,
#                      dt_model = dt_model,
#                      basehz_dt = basehz_dt,
#                      fixed_effects_vec = fixed_effects_vec,
#                      ranef_df = ranef_df,
#                      x_vars = x_vars,
#                      end_times = 1825)
#   }, simplify = FALSE)
...
# 
# library(parallel)
# opos <- unique(recips_dt$OPO)
# 
# no_cores <- max(1, detectCores() - 1)
# cl <- makeCluster(no_cores, type = "FORK") # FORK works on unix-like; use PSOCK on Windows
# 
# clusterExport(cl, c("dt_model", "basehz_dt", "fixed_effects_vec", "ranef_df", "x_vars", "ben_wait_tx_fast"), envir = environment())
# clusterEvalQ(cl, library(data.table))
# clusterEvalQ(cl, library(zoo))
# 
# # For each OPO, compute a matrix of results (recips x 2)
# results_by_opo <- parLapply(cl, opos, function(o) {
#   recs <- recips_dt[OPO == o, PX_ID]
#   res <- sapply(recs, function(px) ben_wait_tx_fast(px, o, dt_model, basehz_dt, fixed_effects_vec, ranef_df, x_vars, end_times = 1825), simplify = "matrix")
#   # sapply with simplify="matrix" returns 2 x N matrix
#   t(res) # return N x 2 matrix
# })
# stopCluster(cl)

```

```{r wait_tx_5_year_gem_v2}
ben_wait_tx <- function(df = to_model,
                        base_hz = basehz,
                        model = six_status, i, j, end_times = 1825){
...
# Clean up OPO ID lookup section:
# Convert i to character for robust lookup (needed because your global OPO names might be factors)
opo_key <- as.character(i)

# Perform the lookup once, using the correct key
opo_i_b0_val <- ref[ref$OPO == opo_key,]$Intercept
opo_i_b1_val <- ref[ref$OPO == opo_key,]$tx

# CRITICAL FIX: Ensure OPO random effects are length 1 and handle NULL/length 0 case
if (length(opo_i_b0_val) == 0) {
 warning(paste0("OPO ", opo_key, " not found in random effects table."))
 return(c(NA, NA))
}
if (length(opo_i_b0_val) > 1) {
 # This should now fire only if there's a serious data issue, not NA coercion
 stop(paste0("OPO ", opo_key, " returned multiple random effects."))
}

# The hazard calculation MUST use the validated variables (opo_i_b0_val/opo_i_b1_val)
d_ij <- d_ij %>% mutate(
	fe_wait = lp_no_tx,
	fe_tx = fe_lp,
	opo_hz_wait = hz_t*exp(fe_wait + opo_i_b0_val),
	opo_hz_tx = hz_t*exp(fe_tx + opo_i_b0_val + opo_i_b1_val)
	) %>%
	mutate(wait_surv = cumprod(1-opo_hz_wait)) %>%
	group_by(tx) %>%
	mutate(
...
ben_wait_tx <- function(df = to_model,
  ...
  print(paste("After initial filter, rows:", nrow(d_ij)))  # DEBUG
  ...
  print(paste("After fixed effects, rows:", nrow(d_ij)))  # DEBUG
  ...
  print(paste("After joining basehz, rows:", nrow(d_ij)))  # DEBUG
  ...
  print(paste("After filter tx==1, rows:", nrow(d_ij)))  # DEBUG
  ...
  print(paste("OPO random effects - Intercept:", opo_i_b0_val, "tx:", opo_i_b1_val))  # DEBUG
  ...
  print(paste("After survival calcs, rows:", nrow(d_ij)))  # DEBUG
  
  d_ij <- d_ij %>%
    filter(time <= max_end_time) %>%
    mutate(surv_benefit = surv_post_tx - wait_surv)
  
  print(paste("After time filter, rows:", nrow(d_ij)))  # DEBUG
  ...
  # Replace this section at the end:
s_benefit <- vector(mode = "numeric", length = 2)

# OLD CODE - looking for exact match:
# wait_5 <- filter(d_ij, time == end_times)$wait_surv
# post_5 <- filter(d_ij, time == end_times)$surv_post_tx

# NEW CODE - find closest time or last observation:
if (end_times %in% d_ij$time) {
  # Exact match exists
  wait_5 <- filter(d_ij, time == end_times)$wait_surv
  post_5 <- filter(d_ij, time == end_times)$surv_post_tx
} else if (max(d_ij$time) < end_times) {
  # Patient doesn't have data out to end_times, use last observation
  wait_5 <- filter(d_ij, time == max(time))$wait_surv
  post_5 <- filter(d_ij, time == max(time))$surv_post_tx
} else {
  # Find closest time point <= end_times
  closest_time <- max(d_ij$time[d_ij$time <= end_times])
  wait_5 <- filter(d_ij, time == closest_time)$wait_surv
  post_5 <- filter(d_ij, time == closest_time)$surv_post_tx
}
...
## Distribution of survival benefit at five years for each OPO (three-status system)
...
OPO_loop <- function(OPOs, recips, cl){
  ...
  for (o in OPOs) {
  	cur_OPO <- o
  	print(paste0("currently calculating for OPO ", o))
  	...
  	colnames(wait) <- c(o, "recips")

  	post_tx <- B_tib %>% select(V2) %>% cbind(recips)
  	colnames(post_tx) <- c(o, "recips")
...
o_list <- row.names(ranef(six_status)[[1]])
o_list <- o_list[!is.na(o_list)]

# o_list_temp <- o_list[1:3]
# by_time_tibbles <- OPO_loop(o_list_temp, r_list, cl)

by_time_tibbles <- OPO_loop(o_list, r_list, cl)
...
write_csv(five_yr_wait, "five_year_waits.csv")
write_csv(five_yr_post, "five_year_posts.csv")
```
...
# 2. Create a sample of recipients for testing (e.g., 100 individuals)
#set.seed(42) # Set seed for reproducibility
#sample_size <- 3 

#recips_sample <- recips %>% 
#  sample_n(sample_size)

# 3. Run marginal calculations on the sample
#print(paste("Calculating marginal hazards for", sample_size, "recipients..."))

# Marginal distribution of benefit of transplant (Hazard Ratio)
#marginal_hazards_sample <- mapply(hr_tx_ij, 
#                                  i = recips_sample$OPO, 
#                                  j = recips_sample$PX_ID)

# Marginal distribution of survival (Wait and Post-Tx)
#marginal_waits_posts_sample <- mapply(ben_wait_tx, 
#                                      i = recips_sample$OPO, 
#                                      j = recips_sample$PX_ID) 

# You can now inspect the results:
#head(marginal_hazards_sample)
#t(head(marginal_waits_posts_sample))

marginal_hazards <- mapply(hr_tx_ij, i = recips$OPO, j = recips$PX_ID)

marginal_waits_posts <- mapply(ben_wait_tx, i = recips$OPO, j = recips$PX_ID)
...
OPO_counts <- recips %>% select(OPO) %>% 
  group_by(OPO) %>% 
  summarise(num_tx = n()) %>% ungroup()

re_effects <- data.frame(ranef(six_status)[[1]])

re_effects$OPO <- row.names(re_effects)

colnames(re_effects)[2] <- "tx_re"

#make an OPO level dataset of random effects
opo_re <- OPO_counts %>% 
  left_join(re_effects, by="OPO") %>% 
...
min_tx_exp <- min(opo_re$tx_exp)

max_tx_exp <- max(opo_re$tx_exp)

min_int_exp <- min(opo_re$intercept_exp)

max_int_exp <- max(opo_re$intercept_exp)
```
...
#make benefit dataframe- hypothetical estimates as if all recipients were transplanted at the OPO
colnames(five_yr_wait) <- c("recips", o_list)
colnames(five_yr_post) <- c("recips", o_list)

M <- merge(five_yr_post,five_yr_wait,by="recips")

ben_five_year_only <- M[,grepl("*\\.x$",names(M))] - M[,grepl("*\\.y$",names(M))]

names(ben_five_year_only) <- gsub(pattern = "\\.x$", replacement = "", x = names(ben_five_year_only))
...
five_year_waits <- five_yr_wait %>% select(-recips) %>% 
  ...
  gather(value = mean_five_year_wait, key = OPO) %>% 
  select(OPO, mean_five_year_wait)

five_year_posts <- five_yr_post %>% select(-recips) %>% 
  summarise_all(mean, na.rm =TRUE) %>% 
  gather(value = mean_five_year_post, key = OPO) %>% 
  select(OPO, mean_five_year_post)


#OPO level dataset of summary statistics for case-mix adjusted 
...
five_year_means <- ben_five_year_only %>% 
  ...
  separate(key, c("OPO", "stat"), sep = "_") %>%
  spread(stat, value) %>% 
  mutate(mean_five_year = mean) %>% select(-mean) %>%
  left_join(five_year_waits, by = "OPO") %>% 
  left_join(five_year_posts, by = "OPO") %>%  
  left_join(OPO_counts, by = "OPO")
...
five_year_means <- five_year_means %>%
  mutate(se_mean = sd/sqrt(num_tx),
         ...
         opo_performance = case_when(
           lower_ci >mean(five_year_means$mean_five_year) ~ "High",
           upper_ci <mean(five_year_means$mean_five_year) ~ "Low",
           TRUE ~ "Average"
         ), opo_performance = factor(opo_performance, 
                                        levels = c("Low", "Average", "High")))

opo_re <- opo_re %>% left_join(five_year_means)
...
save.image("model_fit_18_22_w_benefits.RData")
```
...
catepillar <- ggplot(to_plot, aes(x = index, 
                    ...
                    ymin = lower_ci*100, 
                    ymax = upper_ci*100, 
                    color = opo_performance)) + 
  geom_point() + 
  geom_errorbar() + 
  scale_color_manual(name = "OPO benefit:",
...
num_high <- table(five_year_means$opo_performance)["High"][[1]]

num_low <- table(five_year_means$opo_performance)["Low"][[1]]
...
panel_A <- ggplot(data = opo_re, 
                  aes(x = 100*mean_five_year_wait, 
                      y = 100*mean_five_year)) + 
  geom_point(aes(color = opo_performance), size = 2.5, alpha = 0.9) +
  ...
  scale_y_continuous(name = "OPO five-year surival benefit (%)") + 
                     # limits = c(32, 55), breaks = seq(30, 60, 5)) + 
  scale_color_manual(name = "OPO benefit:", 
                     values = c("red3","black","navyblue")) +
  theme_few() + 
  ggtitle("Panel A") + 
...
panel_B <- ggplot(data = opo_re, 
  ...
  # scale_y_continuous(limits = c(32, 55), breaks = seq(30, 60, 5)) +
...
#for eTable4
cand_thor <- haven::read_sas('/Users/danieljohnson/Library/CloudStorage/Box-Box/SAF Q2 2025/pubsaf2506/cand_thor.sas7bdat', NULL) %>%  
  haven::zap_formats() %>% haven::zap_labels() 
...
post_high_low <- diff_95ci_pct(x = high_low$opo_performance, 
                               y = high_low$mean_five_year_post, 
                               w = high_low$num_tx)

wait_high_low <- diff_95ci_pct(x = high_low$opo_performance, 
                               y = high_low$mean_five_year_wait, 
                               w = high_low$num_tx)

surv_ben_high_low <- diff_95ci_pct(x = high_low$opo_performance, 
...
## eFigure 2: OPO-specific Survival Benefit Framework
...
#transplanted at an OPO with a high benefit from transplant
top_o <- opo_re %>% arrange(-benefit) %>% filter(row_number()==1)
top_o_tx_hr <- top_o$tx_exp
top_o_wt_risk <- top_o$intercept_exp
top_o <- top_o$OPO

best_opo_recips <- unique(to_model %>% filter(OPO == top_o) %>% 
                            select(PX_ID)) %>% 
...
ex_pt_top <- best_opo_recips[[83, "PX_ID"]]

t_tx <- best_opo_recips[[83, "t_1"]]

hz_good <- exp(hr_tx_ij(model = six_status, 
                        df = to_model, i = top_o, j = ex_pt_top))

max_fup <- max(basehz$time)/365

ex_benefit <- ben_wait_tx(df = to_model, 
                          base_hz = basehz, 
                          model = six_status, j = ex_pt_top, i = top_o)
...
good_plot <- b_ij(df = to_model, base_hz = basehz, model = six_status, j = ex_pt_top, i = top_o) + 
...
#transplanted at an OPO with a lower benefit from transplant
worst_o <- opo_re %>% arrange(benefit) %>% filter(row_number()==1)
worst_o_tx_hr <- worst_o$tx_exp
worst_o_wt_risk <- worst_o$intercept_exp
worst_o <- worst_o$OPO

worst_o_recips <- unique(to_model %>% 
                           filter(OPO == worst_o) %>% 
                           select(PX_ID)) %>% 
  left_join(to_model %>% select(PX_ID, tx, stat_1a, t_1, era_tx) %>% 
              group_by(PX_ID, tx) %>% 
...
ex_worst <- worst_o_recips[[368,"PX_ID"]]

t_tx_worst <- worst_o_recips[[368,"t_1"]]
...
hz_bad <- exp(hr_tx_ij(model = six_status, df = to_model, i = worst_o, j = ex_worst))


bad_plot <- b_ij(df = to_model, 
                 base_hz = basehz, 
                 model = six_status,  
                 i = worst_o, j = ex_worst)+
...
## OPO Tier Analysis
```{r opo_tier_analysis}
# Read in the OPO tier data
opo_tiers <- read.csv("opo_tiers.csv")

# Reshape from wide to long format
library(tidyr)
opo_tiers_long <- opo_tiers %>%
  pivot_longer(cols = starts_with("tier"),
               names_to = "year_tier",
               values_to = "tier") %>%
  mutate(year = as.numeric(paste0("20", str_extract(year_tier, "\\d+"))),
         tier = factor(tier, levels = c(1, 2, 3)))

# Extract transplant year from tx_hr using REC_TX_DT
tx_year_data <- tx_hr %>%
  filter(PX_ID %in% to_model$PX_ID) %>%
  mutate(tx_year = as.numeric(format(REC_TX_DT, "%Y"))) %>%
  select(PX_ID, tx_year, REC_OPO_ID)

# Get unique transplanted patients with their OPO
recips_with_year <- to_model %>%
  filter(tx == 1) %>%
  group_by(PX_ID) %>%
  filter(row_number() == 1) %>%
  ungroup() %>%
  select(PX_ID, OPO) %>%
  left_join(tx_year_data %>% select(PX_ID, tx_year), by = "PX_ID")

# Link to the survival benefit calculations
recips_with_benefit <- recips_with_year %>%
  left_join(pop_tx_hz_6_stat %>% select(PX_ID, benefit_6), by = "PX_ID")

# Join with OPO tier data
recips_with_tier <- recips_with_benefit %>%
  left_join(opo_tiers_long %>% select(OPO.Code, year, tier),
            by = c("OPO" = "OPO.Code", "tx_year" = "year")) %>%
  filter(!is.na(tier)) %>%
  filter(!is.na(benefit_6)) %>%
  filter(benefit_6 > 0)  # Only include positive benefits

# Summary statistics by tier
tier_summary <- recips_with_tier %>%
  group_by(tier) %>%
  summarise(
    n = n(),
    mean_benefit = mean(benefit_6, na.rm = TRUE),
    median_benefit = median(benefit_6, na.rm = TRUE),
    sd_benefit = sd(benefit_6, na.rm = TRUE),
    q25 = quantile(benefit_6, 0.25, na.rm = TRUE),
    q75 = quantile(benefit_6, 0.75, na.rm = TRUE)
  )

print(tier_summary)

# Create labels with sample sizes
recips_with_tier <- recips_with_tier %>%
  mutate(tier_label = factor(tier,
                              levels = c(1, 2, 3),
                              labels = c(paste0("Tier 1\nN = ", 
                                                comma(filter(tier_summary, tier == 1)$n)),
                                         paste0("Tier 2\nN = ", 
                                                comma(filter(tier_summary, tier == 2)$n)),
                                         paste0("Tier 3\nN = ", 
                                                comma(filter(tier_summary, tier == 3)$n)))))

# Create box and whisker plot matching your Figure 2 style
library(ggplot2)
library(ggthemes)
library(grid)
library(gridExtra)

box_plot <- ggplot(recips_with_tier, aes(x = tier_label, y = 100*benefit_6)) + 
  geom_boxplot() +
  stat_boxplot(geom = "errorbar", width = 0.5) +
  geom_point(data = tier_summary, 
             aes(x = factor(tier, 
                            levels = c(1, 2, 3),
                            labels = c(paste0("Tier 1\nN = ", comma(filter(tier_summary, tier == 1)$n)),
                                       paste0("Tier 2\nN = ", comma(filter(tier_summary, tier == 2)$n)),
                                       paste0("Tier 3\nN = ", comma(filter(tier_summary, tier == 3)$n)))),
                 y = 100*mean_benefit, 
                 shape = "mean"), 
             size = 4, fill = "gray", alpha = 0.5) +
  scale_shape_manual(values = 23) +
  labs(x = " ",
       y = "Absolute five-year survival benefit (%)",
       shape = " ") +
  theme_few() +
  theme(axis.title = element_text(),
        axis.title.y = element_text(),
...
# Save the plot
ggsave("opo_tier_survival_benefit.pdf", 
       plot = box_plot, 
       width = 6, 
       height = 4)

# Statistical test - ANOVA
anova_result <- aov(benefit_6 ~ tier, data = recips_with_tier)
anova_summary <- summary(anova_result)
print(anova_summary)

# Post-hoc pairwise comparisons
pairwise_result <- pairwise.t.test(recips_with_tier$benefit_6, 
                                   recips_with_tier$tier,
                                   p.adjust.method = "bonferroni")
print(pairwise_result)

# Kruskal-Wallis test (non-parametric alternative)
kruskal_result <- kruskal.test(benefit_6 ~ tier, data = recips_with_tier)
print(kruskal_result)

# Create summary text for results section
cat("\n=== Results Summary ===\n")
cat(paste0("Recipients matched to OPO tiers: ", comma(nrow(recips_with_tier)), "\n"))
cat(paste0("Tier 1: Mean benefit = ", comma_2(100*filter(tier_summary, tier == 1)$mean_benefit), 
           "%, Median = ", comma_2(100*filter(tier_summary, tier == 1)$median_benefit), "%\n"))
cat(paste0("Tier 2: Mean benefit = ", comma_2(100*filter(tier_summary, tier == 2)$mean_benefit), 
           "%, Median = ", comma_2(100*filter(tier_summary, tier == 2)$median_benefit), "%\n"))
cat(paste0("Tier 3: Mean benefit = ", comma_2(100*filter(tier_summary, tier == 3)$mean_benefit), 
           "%, Median = ", comma_2(100*filter(tier_summary, tier == 3)$median_benefit), "%\n"))
cat(paste0("ANOVA p-value: ", comma_p(anova_summary[[1]]$`Pr(>F)`[1]), "\n"))
```
...
# Define the file path (update this to your actual file location)
file_path <- "/Users/danieljohnson/Library/CloudStorage/OneDrive-TheUniversityofChicago/Projects/OPO/opo_survival/VS17MORT.DUSMCPUB"

# 1. Define the column positions based on the NCHS 2017 Data User Guide
# Note: "start" and "end" are inclusive 1-based indices.
mortality_layout <- fwf_positions(
  start = c(19, 20, 65, 69, 70, 75, 77, 79, 102, 146, 445),
  end   = c(19, 20, 66, 69, 73, 76, 78, 80, 105, 149, 446),
  col_names = c(
    "Record_Type",      # 1=Resident, 2=Non-resident
    "Resident_Status",  # 1=Res, 2=Intrastate Non-res, 3=Interstate, 4=Foreign
    "Month_of_Death",   # 01-12
    "Sex",              # M/F
    "Detail_Age",       # First digit is age unit (1=Years), next 3 are value
    "Age_Recode_52",    # 52 groups
    "Age_Recode_27",    # 27 groups
    "Age_Recode_12",    # 12 groups
    "Year",             # 2017
    "UCOD_ICD10",       # Underlying Cause of Death (ICD-10)
    "Race_Recode_3"     # 1=White, 2=Black, 3=Other, etc.
  )
)

# 2. Read the file
# This file is large (~3 million rows), so 'lazy = TRUE' helps by not loading everything at once if you don't need to.
df_mortality <- read_fwf(
  file = file_path,
  col_positions = mortality_layout,
  col_types = cols(.default = col_character()) # Read all as text first to avoid parsing errors
)

# 3. Quick Clean Up (Optional)
# Convert Age columns to numeric where appropriate or handle the "Detail Age" logic
df_final <- df_mortality %>%
  mutate(
    Year = as.integer(Year),
    Month_of_Death = as.integer(Month_of_Death)
  )
